<?php

namespace AppBundle\GameEngine\GameData\ClubsByCountry;

class ClubsGermany
{
    public static function clubs()
    {
        return [
            [
                'name' => 'Juventus',
                'league_id' => 3,
                'country' => 3,
                'city' => 29,
                'coefficient' => 126, // uefa competitions coefficient
                'coeff1' => 23, // uefa coefficient for the last year to last five years
                'coeff2' => 29,
                'coeff3' => 18,
                'coeff4' => 33,
                'coeff5' => 23,
            ],
            [
                'name' => 'Napoli',
                'league_id' => 3,
                'country' => 3,
                'city' => 30,
                'coefficient' => 78, // uefa competitions coefficient
                'coeff1' => 16, // uefa coefficient for the last year to last five years
                'coeff2' => 22,
                'coeff3' => 13,
                'coeff4' => 17,
                'coeff5' => 10,
            ],
            [
                'name' => 'Inter Milan',
                'league_id' => 3,
                'country' => 3,
                'city' => 31,
                'coefficient' => 16, // uefa competitions coefficient
                'coeff1' => 0, // uefa coefficient for the last year to last five years
                'coeff2' => 12,
                'coeff3' => 0,
                'coeff4' => 4,
                'coeff5' => 0,
            ],
            [
                'name' => 'AC Milan',
                'league_id' => 3,
                'country' => 3,
                'city' => 31,
                'coefficient' => 28, // uefa competitions coefficient
                'coeff1' => 16, // uefa coefficient for the last year to last five years
                'coeff2' => 0,
                'coeff3' => 0,
                'coeff4' => 0,
                'coeff5' => 12,
            ],
            [
                'name' => 'Atalanta',
                'league_id' => 3,
                'country' => 3,
                'city' => 32,
                'coefficient' => 11, // uefa competitions coefficient
                'coeff1' => 0, // uefa coefficient for the last year to last five years
                'coeff2' => 0,
                'coeff3' => 0,
                'coeff4' => 0,
                'coeff5' => 11,
            ],
            [
                'name' => 'AS Roma',
                'league_id' => 3,
                'country' => 3,
                'city' => 33,
                'coefficient' => 64, // uefa competitions coefficient
                'coeff1' => 0, // uefa coefficient for the last year to last five years
                'coeff2' => 12,
                'coeff3' => 14,
                'coeff4' => 13,
                'coeff5' => 25,
            ],
            [
                'name' => 'SS Lazio',
                'league_id' => 3,
                'country' => 3,
                'city' => 33,
                'coefficient' => 41, // uefa competitions coefficient
                'coeff1' => 10, // uefa coefficient for the last year to last five years
                'coeff2' => 0,
                'coeff3' => 14,
                'coeff4' => 0,
                'coeff5' => 17,
            ],
            [
                'name' => 'Torino',
                'coefficient' => 13, // uefa competitions coefficient
                'coeff1' => 0, // uefa coefficient for the last year to last five years
                'coeff2' => 13,
                'coeff3' => 0,
                'coeff4' => 0,
                'coeff5' => 0,
            ],
            [
                'name' => 'UC Sampdoria',
                'coefficient' => 1, // uefa competitions coefficient
                'coeff1' => 0, // uefa coefficient for the last year to last five years
                'coeff2' => 0,
                'coeff3' => 1,
                'coeff4' => 0,
                'coeff5' => 0,
            ],
            [
                'name' => 'Fiorentina',
                'coefficient' => 54, // uefa competitions coefficient
                'coeff1' => 15, // uefa coefficient for the last year to last five years
                'coeff2' => 20,
                'coeff3' => 8,
                'coeff4' => 11,
                'coeff5' => 0,
            ],
            [
                'name' => 'US Sassuolo Calcio',
                'coefficient' => 4, // uefa competitions coefficient
                'coeff1' => 0, // uefa coefficient for the last year to last five years
                'coeff2' => 0,
                'coeff3' => 0,
                'coeff4' => 4,
                'coeff5' => 0,
            ],
            [
                'name' => 'Cagliari',
                'coefficient' => 0, // uefa competitions coefficient
                'coeff1' => 0, // uefa coefficient for the last year to last five years
                'coeff2' => 0,
                'coeff3' => 0,
                'coeff4' => 0,
                'coeff5' => 0,
            ],
            [
                'name' => 'Genoa',
                'coefficient' => 0, // uefa competitions coefficient
                'coeff1' => 0, // uefa coefficient for the last year to last five years
                'coeff2' => 0,
                'coeff3' => 0,
                'coeff4' => 0,
                'coeff5' => 0,
            ],
            [
                'name' => 'Parma',
                'coefficient' => 0, // uefa competitions coefficient
                'coeff1' => 0, // uefa coefficient for the last year to last five years
                'coeff2' => 0,
                'coeff3' => 0,
                'coeff4' => 0,
                'coeff5' => 0,
            ],
            [
                'name' => 'Udinese',
                'coefficient' => 2, // uefa competitions coefficient
                'coeff1' => 2, // uefa coefficient for the last year to last five years
                'coeff2' => 0,
                'coeff3' => 0,
                'coeff4' => 0,
                'coeff5' => 0,
            ],
            [
                'name' => 'SPAL',
                'coefficient' => 0, // uefa competitions coefficient
                'coeff1' => 0, // uefa coefficient for the last year to last five years
                'coeff2' => 0,
                'coeff3' => 0,
                'coeff4' => 0,
                'coeff5' => 0,
            ],
            [
                'name' => 'Bologna',
                'coefficient' => 0, // uefa competitions coefficient
                'coeff1' => 0, // uefa coefficient for the last year to last five years
                'coeff2' => 0,
                'coeff3' => 0,
                'coeff4' => 0,
                'coeff5' => 0,
            ],
            [
                'name' => 'Empoli',
                'coefficient' => 0, // uefa competitions coefficient
                'coeff1' => 0, // uefa coefficient for the last year to last five years
                'coeff2' => 0,
                'coeff3' => 0,
                'coeff4' => 0,
                'coeff5' => 0,
            ],
            [
                'name' => 'Frosinone',
                'coefficient' => 0, // uefa competitions coefficient
                'coeff1' => 0, // uefa coefficient for the last year to last five years
                'coeff2' => 0,
                'coeff3' => 0,
                'coeff4' => 0,
                'coeff5' => 0,
            ],
            [
                'name' => 'Chievo',
                'coefficient' => 0, // uefa competitions coefficient
                'coeff1' => 0, // uefa coefficient for the last year to last five years
                'coeff2' => 0,
                'coeff3' => 0,
                'coeff4' => 0,
                'coeff5' => 0,
            ],
        ];
    }
}